...
int main() {
    ...
    while(1) {
        ...
        switch(page)
        {
            case 1:   // Draw images
                ...
                if(wait > TILE_DELAY) {
                    ...
                    wait = 0;
                    if(wpadheld & PAD_BUTTON_LEFT || wpadheld & PAD_BUTTON_RIGHT ||
                        wpadheld & PAD_BUTTON_UP || wpadheld & PAD_BUTTON_DOWN) {
                        frame++;
                    }
                    else {
        ...
        if(wpadheld & PAD_BUTTON_LEFT) {
            if(wpadheld & PAD_BUTTON_B || page == 1)
                left -= 2;
            else
                left--;
            direction_new = TILE_LEFT;  // for tile example
        }
        if(wpadheld & PAD_BUTTON_RIGHT) {
            if(wpadheld & PAD_BUTTON_B || page == 1)
                left += 2;
            else
                left++;
        ...
        if(wpaddown & PAD_TRIGGER_L) {
            page--;
            left = 0;
            top = 0;
            if(page < 0) page = 2;
        }
        if(wpaddown & PAD_TRIGGER_R) {
            page++;
            left = 0;
            top = 0;
            if(page > 2) page = 0;
        }
        if(wpadheld & PAD_BUTTON_X && wpadheld & PAD_BUTTON_Y) {
